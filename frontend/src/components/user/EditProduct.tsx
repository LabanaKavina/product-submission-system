  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      if (data.status !== 'Submitted') {
        setLoadError(`This product cannot be edited because its status is "${data.status}". Only "Submitted" products can be edited.`);
        setCanEdit(false);
        return;
      }
      setCanEdit(true);
          ...
          <CardBody>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-red-700">{loadError || 'This product cannot be edited.'}</p>
              </div>
            </div>
          </CardBody>
        </Card>
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Update the details below. Changes are saved when you click "Save Changes".
        </p>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur border-t border-gray-200 shadow-[0_-2px_12px_rgba(0,0,0,0.06)]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-3">
          <p className="text-xs text-gray-400 hidden sm:block">
            Saving will update the product and keep it in "Submitted" status.
          </p>
          <div className="flex items-center gap-3 ml-auto">
            <Button type="button" variant="secondary" onClick={() => navigate(`/user/products/${id}`)} disabled={isSubmitting}>
              Cancel
            </Button>
            <Button form="edit-product-form" type="submit" variant="primary" isLoading={isSubmitting}>
