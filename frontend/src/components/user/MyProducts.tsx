...
export default function MyProducts() {
  ...
  const [listState, setListState] = useState<{
    ...
    isLoading: boolean;
    error: string | null;
  }>({ products: [], pagination: { total: 0, page: 1, limit: 10, totalPages: 1 }, isLoading: true, error: null });

  const [query, setQuery] = useState({ search: '', statusFilter: '' as StatusFilter, page: 1 });
  ...
  const prevFilters = useRef({ search: debouncedSearch, status: query.statusFilter });
  ...
  useEffect(() => {
    const filterChanged =
      ...
      prevFilters.current.status !== query.statusFilter;
    if (filterChanged) {
      prevFilters.current = { search: debouncedSearch, status: query.statusFilter };
      pageRef.current = 1;
      setQuery(q => ({ ...q, page: 1 }));
    }
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      setListState(s => ({ ...s, isLoading: true, error: null }));
      try {
        ...
        if (!cancelled) {
          setListState({
            ...
            pagination: data.pagination ?? { total: 0, page: pageRef.current, limit: 10, totalPages: 1 },
            isLoading: false,
            error: null,
          });
        }
      } catch (err) {
        if (!cancelled) setListState(s => ({ ...s, isLoading: false, error: err instanceof Error ? err.message : 'Failed to load products' }));
      }
    };

    doFetch();
    return () => { cancelled = true; };
  }, [debouncedSearch, query.statusFilter, token, fetchTrigger]); // eslint-disable-line react-hooks/exhaustive-deps

  const { products, pagination, isLoading, error } = listState;
  const { page, statusFilter } = query;
